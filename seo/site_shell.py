...
def filter_script(selector:str)->str:
    return f'''<script>function filterCatalog(q){{q=q.toLowerCase().trim();document.querySelectorAll('{selector}').forEach(function(x){{x.style.display=!q||x.textContent.toLowerCase().includes(q)?'':'none';}});}}</script>'''
...
def copy_assets(root:Path)->None:
    ...
    if source.exists():
        dest=root/'assets';dest.mkdir(parents=True,exist_ok=True);shutil.copy2(source,dest/'trial-finder-promo.jpg')
