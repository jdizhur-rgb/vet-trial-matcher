...
def page(title,desc,body,canonical,lang='en',alts=None):
    ...
    css += '.center-region{margin:10px 0;border:1px solid #d9e2ea;border-radius:10px;background:#fff;overflow:hidden}.center-region>summary{cursor:pointer;list-style:none;padding:12px 14px;color:#315f7d;font-size:1.05rem;font-weight:750}.center-region>summary::-webkit-details-marker{display:none}.center-region>summary:after{content:"+";float:right;font-size:1.18rem;font-weight:500}.center-region[open]>summary:after{content:"−"}.center-region-count{display:inline-block;margin-left:7px;color:#607086;font-size:.78rem;font-weight:600}.center-region>.center-directory-grid{padding:0 12px 12px}'
...
def generate_centers(rows):
    ...
    def region_group(label,group):
        return (f'<details class="center-region"><summary>{g.esc(label)}'
            f'<span class="center-region-count">{len(group)}</span></summary>'
            '<div class="center-directory-grid">'+''.join(x['card'] for x in group)+'</div></details>')
    ...
    def usa_section():
        ...
        for region in sorted({x['region'] for x in selected},key=lambda value:(value.startswith(('National','Multiple')),state_names.get(value,value))):
            group=sorted((x for x in selected if x['region']==region),key=lambda x:x['name'])
            out.append(region_group(state_names.get(region,region),group))
    ...
    def directory_section(key,title,intro):
        ...
        for country in sorted({x['country'] for x in selected}):
            group=sorted((x for x in selected if x['country']==country),key=lambda x:x['name'])
            out.append(region_group(country,group))
        out.append('</section>');return ''.join(out)
    ...
    ib=('<h1>Oncology centers &amp; research programs</h1><p class="lead">Browse the institutions and participating hospitals connected to current cancer treatment opportunities. Hospitals in the United States are grouped by state.</p>'
        '<nav class="center-directory-nav" aria-label="Center types"><a href="#usa">USA by state</a><a href="#universities">Universities &amp; teaching hospitals</a><a href="#hospitals">Specialty hospitals</a><a href="#other">Research centers &amp; multicenter programs</a></nav>'
        +usa_section()
