...
def center_overview(center):
    if center == CSU_CENTER:
        return (
            ...
            '<figcaption style="font-size:.86rem;color:#607086;margin-top:7px">Colorado State University Translational Medicine Institute, home to research programs that collaborate with the Flint Animal Cancer Center. Photo: Colorado State University.</figcaption></figure>'
            ...
            '<p><a href="https://vetmedbiosci.colostate.edu/cs/research-topic-directory/comparative-oncology-and-cancer-biology/" rel="noopener">Meet CSU comparative oncology researchers</a> · <a href="https://vetmedbiosci.colostate.edu/vth/clinical_trial_tag/oncology/" rel="noopener">See CSU oncology clinical trials</a></p></div>'
        )
    profile = PROFILES.get(center)
    if profile:
        links = ' · '.join(f'<a href="{g.esc(url)}" rel="noopener">{g.esc(label)}</a>' for label, url in profile.get("links", []))
        return (
            '<div class="center-overview" style="text-align:justify;text-justify:inter-word">'
            f'<h2 style="text-align:left">{g.esc(profile["title"])}</h2><p>{g.esc(profile["about"])}</p>'
            f'<h2 style="text-align:left">Cancer research &amp; team</h2><p>{g.esc(profile["research"])}</p>'
            + (f'<p style="text-align:left">{links}</p>' if links else '') + '</div>'
        )
    return ('<div class="center-overview" style="text-align:justify;text-justify:inter-word"><h2 style="text-align:left">About this veterinary cancer research center</h2>'
            f'<p><strong>{g.esc(center)}</strong> currently has veterinary cancer treatment or research opportunities represented in our catalog. The listings below are tied to the individual study teams and official study sources, so owners can review the actual treatment approach, eligibility details and contact information without a paid matching report.</p></div>')
...
def generate_center_pages():
    ...
    center_links, index_items = [], []; used_slugs = {}
    for center, hit in sorted(grouped.items()):
        base_slug = g.slugify(center.replace("College of Veterinary Medicine", "").replace("School of Veterinary Medicine", "")) or "research-center"; slug = base_slug
        if slug in used_slugs and used_slugs[slug] != center: slug = g.slugify(center)
        used_slugs[slug] = center; path = f"centers/{slug}/"; url = f"{g.SITE}/{path}"
        cancers = sorted({c for r in hit for c in row_cancers(r)}); cancer_text = ", ".join(g.display_name(c) for c in cancers) if cancers else "multiple cancer types"
        ...
        dest = g.OUT / path; dest.mkdir(parents=True, exist_ok=True); (dest / "index.html").write_text(g.page(h1, f"Current veterinary cancer clinical trials and treatment studies at {center}.", body, url), encoding="utf-8")
    ...
    center_dir = g.OUT / "centers"; center_dir.mkdir(parents=True, exist_ok=True); (center_dir / "index.html").write_text(g.page("Veterinary Cancer Clinical Trial Centers", "U.S. universities, veterinary hospitals and cancer research centers with current treatment opportunities.", index_body, index_url), encoding="utf-8")
    sitemap = g.OUT / "sitemap.xml"; text = sitemap.read_text(encoding="utf-8"); additions = ''.join(f'<url><loc>{g.esc(u)}</loc></url>\n' for u in [index_url] + center_links); sitemap.write_text(text.replace('</urlset>', additions + '</urlset>'), encoding="utf-8")
