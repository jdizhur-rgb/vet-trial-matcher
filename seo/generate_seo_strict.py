...
def page(title,desc,body,canonical,lang='en',alts=None):
    ...
    css += '.center-directory-nav button{border:1px solid #d5e2ea;border-radius:999px;padding:8px 12px;background:#edf4f8;color:#315f7d;font:650 1rem/1.3 system-ui,-apple-system,sans-serif;cursor:pointer}.center-directory-nav button[aria-pressed="true"]{background:#315f7d;color:#fff;border-color:#315f7d}.center-directory-section[hidden],.center-region[hidden]{display:none!important}.center-region-picker{display:grid;gap:7px;max-width:430px;margin:16px 0 20px;color:#315f7d;font-weight:750}.center-region-picker select{width:100%;box-sizing:border-box;padding:11px 13px;border:1px solid #b8c8d6;border-radius:9px;background:#fff;color:#17243b;font:inherit}.center-region{margin:14px 0}.center-region>h3{margin:0 0 10px}.center-region-count{display:inline-block;margin-left:7px;color:#607086;font-size:.78rem;font-weight:600}@media(max-width:700px){.center-directory-nav button{width:100%;text-align:center}}'
...
def generate_centers(rows):
    ...
    def region_group(value,label,group):
        return (f'<div class="center-region" data-region="{g.esc(value)}" hidden>'
            f'<h3>{g.esc(label)} <span class="center-region-count">{len(group)}</span></h3>'
            '<div class="center-directory-grid">'+''.join(x['card'] for x in group)+'</div></div>')

    def region_picker(section_id,choices,label):
        options=''.join(f'<option value="{g.esc(value)}">{g.esc(name)}</option>' for value,name in choices)
        return (f'<label class="center-region-picker" for="{section_id}-region">{g.esc(label)}'
            f'<select id="{section_id}-region" data-region-picker><option value="">Choose one</option>{options}</select></label>')

    def usa_section():
        selected=[x for x in item_data if x['country']=='USA']
        out=['<section class="center-directory-section" id="usa"><h2>United States by state</h2><p>Open a state to see every listed university, specialty hospital and research center there.</p>']
        regions=sorted({x['region'] for x in selected},key=lambda value:(value.startswith(('National','Multiple')),state_names.get(value,value)))
        out.append(region_picker('usa',[(region,state_names.get(region,region)) for region in regions],'State'))
        for region in regions:
            group=sorted((x for x in selected if x['region']==region),key=lambda x:x['name'])
            out.append(region_group(region,state_names.get(region,region),group))
    ...
    def directory_section(key,title,intro):
        ...
        for country in countries:
            group=sorted((x for x in selected if x['country']==country),key=lambda x:x['name'])
            out.append(region_group(country,country,group))
        out.append('</section>');return ''.join(out)
