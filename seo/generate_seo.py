...
'it':('Studi clinici e studi sul trattamento del cancro','Opportunità terapeutiche attualmente disponibili','Cerca le opzioni di trattamento attuali'),
...
CANONICAL_RULES=(
 ...
 ('brain tumor', ('brain tumor','brain tumour','glioma','glioblastoma')),
 ('nasal tumor', ('nasal tumor','nasal tumour','sinonasal')),
 ('lung cancer', ('lung cancer','pulmonary carcinoma')),
...
def load_effective():
 ...
 # Upserts override base, but delete markers have FINAL precedence. Previously deletes
 # were applied first, then deleted upserts were accidentally resurrected in SEO.
...
def cards(rows): return ''.join(f'<article><h3>{esc(r.get("title"))}</h3><p><strong>{esc(r.get("center"))}</strong> · {esc(r.get("country"))}</p><p>{esc(r.get("status"))}</p></article>' for r in rows)

def main():
 rows=load_effective();OUT.exists() and shutil.rmtree(OUT);OUT.mkdir(parents=True);links=[];index=[];cancers=sorted({c for r in rows for c in row_cancers(r)})
  ...
  for skey,sname in SPECIES.items():
   ...
   for key in cancers:
    ...
    for lang in langs:
     ...
     h1=f'{label}: Treatment Options and Clinical Trials for {sname}s in {region_label}' if lang=='en' else f'{label}: {LANGS[lang][0]} — {sname}s, {region_label}'
     desc=f'Current {label} treatment options, clinical trials, advanced and experimental cancer treatments for {sname.lower()}s in {region_label}.' if lang=='en' else f'{LANGS[lang][1]}: {label} — {sname}s, {region_label}.'
     body=f'<h1>{esc(h1)}</h1><p>{esc(desc)}</p><p><strong>{len(hit)}</strong> treatment-focused opportunities.</p>{cards(hit)}';dest=OUT/path;dest.mkdir(parents=True,exist_ok=True);(dest/'index.html').write_text(page(h1,desc,body,url,lang,alternates));links.append(url);index.append((label,sname,region_label,url))
 root_title='Cancer Treatment Options & Clinical Trials for Dogs and Cats';root_desc='Free finder for current dog and cat cancer treatment options, clinical trials, advanced treatments and experimental therapies.';root_body='<h1>Cancer Treatment Options and Clinical Trials for Dogs and Cats</h1><p>Find current canine and feline cancer treatment options, clinical trials, advanced treatments and experimental therapies by diagnosis and region.</p><ul>'+''.join(f'<li><a href="{u}">{esc(c)} — {esc(s)} — {esc(r)}</a></li>' for c,s,r,u in index if '/de/' not in u and '/fr/' not in u and '/es/' not in u and '/it/' not in u and '/nl/' not in u)+'</ul>';(OUT/'index.html').write_text(page(root_title,root_desc,root_body,SITE+'/'))
 links=[SITE+'/']+links;(OUT/'sitemap.xml').write_text('<?xml version="1.0" encoding="UTF-8"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">'+''.join(f'<url><loc>{esc(u)}</loc></url>' for u in links)+'</urlset>');(OUT/'robots.txt').write_text(f'User-agent: *\nAllow: /\nSitemap: {SITE}/sitemap.xml\n');print(f'Generated {len(links)} indexable pages from {len(rows)} current treatment opportunities using {len(cancers)} canonical cancer types')
