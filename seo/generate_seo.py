...
def page(title,desc,body,canonical,lang='en',alternates=None):
 ...
 return f'''<!doctype html><html lang="{lang}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{esc(title)}</title><meta name="description" content="{esc(desc)}"><link rel="canonical" href="{canonical}">{alts}<style>body{{font-family:system-ui,sans-serif;max-width:920px;margin:auto;padding:28px;line-height:1.55;color:#17243b}}a{{color:#175b8c}}article{{border-top:1px solid #d9e2ea;padding:18px 0}}article h3{{margin-bottom:5px}}.count{{font-size:1.12rem}}.status{{font-weight:600}}.cta,.trial-link{{display:inline-block;padding:10px 15px;background:#287fb8;color:white;text-decoration:none;border-radius:8px}}.trial-link{{padding:7px 11px;font-size:.92rem;background:#3b8fc4}}.meta{{color:#4d5967}}small{{color:#596674}}</style></head><body><header><a href="{SITE}/"><strong>Cancer Trial Finder For Dogs And Cats</strong></a><p>Free. No registration, email or paywall.</p></header><main>{body}<p><small>Listings change. Final eligibility and enrollment decisions are made by each research team. Always confirm current recruiting status with the study team.</small></p></main></body></html>'''
  ...
  for skey,sname in SPECIES.items():
   ...
   for key in cancers:
    ...
    for lang in langs:
     ...
     noun='trial' if len(hit)==1 else 'trials'; body=f'<h1>{esc(h1)}</h1><p class="count"><strong>{len(hit)} current treatment {noun}</strong> in our catalog. Trial names, locations and source links are shown below — free, with no paywall.</p><p><a class="cta" href="{FINDER}">{esc(LANGS[lang][2])}</a></p>{cards(hit)}'
