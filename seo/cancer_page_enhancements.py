...
DISEASE_RE=re.compile(r'<section class="disease">.*?</section>',re.S); CANONICAL_RE=re.compile(r'<link rel="canonical" href="([^"]+)">')
...
def _enhance(text):
 ...
 count=lead.group('count');canonical=html.unescape(cm.group(1));text=text.replace(m.group(0),f'<div class="cancer-page"><h1>{label_html}</h1><p class="eyebrow">{species} · {region_html}</p>',1)
