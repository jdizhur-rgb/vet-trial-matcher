...
def _enhance(text):
 ...
 # Remove the old large FREE promo + CTA; reinsert a compact transition after the owner guidance.
 ...
 # If prior enhancement wording is present, replace it too.
 text=re.sub(r'<h2>Current clinical trials &amp; treatment options</h2><p class="section-intro">.*?</p>',transition,text,count=1,flags=re.S)
 # Move disease guidance before the transition if generation order changes.
