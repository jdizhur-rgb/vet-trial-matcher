...
def _enhance(text):
 ...
 text=text.replace(m.group(0),f'<div class="cancer-page"><h1>{label_html}</h1><p class="eyebrow">{species} · {region_html} · Clinical trials &amp; treatment options</p>',1)
 if count=='0': summary='<div class="cancer-summary"><strong class="opportunity-count">No current treatment opportunities in our catalog</strong>'+f'<p>We do not currently have an active {label_html} trial or advanced treatment listing for this species and region.</p>'+'<p class="summary-detail">The cancer information on this page remains available, and new studies can be added here when enrollment opens.</p></div>'
 else: summary=f'<div class="cancer-summary"><strong class="opportunity-count">{count} current treatment opportunities</strong>'+f'<p>Clinical trials and advanced cancer treatment studies currently represented in our catalog for {label_html}.</p>'+'<p class="summary-detail">See locations, key eligibility, costs when provided, contacts and official enrollment links below.</p></div>'
 text=LEAD_RE.sub(summary,text,count=1); info=CONTENT.get(key)
 if not info: raise AssertionError(f'Missing owner-facing cancer content for {label}')
 about,treatment,factors=info; disease=f'<section class="disease"><h2>About {label_html}</h2><p>{html.escape(about)}</p><h2>How it is usually treated</h2><p>{html.escape(treatment)}</p><h2>What can affect treatment choices</h2><p>{html.escape(factors)}</p></section>'; text,n=DISEASE_RE.subn(disease,text,count=1)
 if n!=1:raise AssertionError(f'Could not replace disease section for {label}')
 dm=DISEASE_RE.search(text)
 if dm:
  block=dm.group(0); text=text[:dm.start()]+text[dm.end():]; anchor=text.find('<div class="cancer-summary">')
  if anchor<0:raise AssertionError(f'Could not locate cancer summary for {label}')
  text=text[:anchor]+block+text[anchor:]
 ri='<h2>Current clinical trials &amp; treatment options</h2><p class="section-intro">No active listings are currently represented in our catalog for this species and region. We keep this page available so new opportunities can appear here when they open.</p>' if count=='0' else '<h2>Current clinical trials &amp; treatment options</h2><p class="section-intro">Browse the current listings below, then use the free matcher to check the study-specific criteria against your pet’s diagnosis and situation.</p>'
 text=text.replace('<h2>Treatment &amp; research</h2><p>Below are treatment-focused clinical trials and advanced oncology options currently represented in our live catalog.</p>',ri,1); related=RELATED.get(key,())
 if related:
