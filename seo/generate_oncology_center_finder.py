...
FILTERS = (
    "Medical oncology", "Chemotherapy", "Surgical oncology", "Radiation oncology", "ECT",
    "Immunotherapy", "Targeted therapy", "SRT", "SBRT", "Interventional radiology", "Clinical trials",
)
...
def card(center: dict) -> str:
    ...
    return (
        ...
        f'<p class="distance" hidden></p>{phone}<div class="tags">{tags}</div>'
        f'<p><a href="{esc(center["website"])}" target="_blank" rel="noopener">Official website →</a></p></article>'
    )


def portal(centers: list[dict]) -> str:
    options = '<option value="">All services</option>' + "".join(
        f'<option value="{esc(normalized(x))}">{esc(x)}</option>' for x in FILTERS
    )
    cards = "".join(card(x) for x in centers)
    return f'''<section class="portal"><nav class="portal-nav"><a href="../">🐾 Clinical trials</a><a href="../centers/">🏥 Oncology centers</a><a href="../advanced/">🧬 Advanced treatments</a><a href="../expanded-access/">🧪 Expanded access</a></nav><h1>Find veterinary oncology centers near you</h1><p class="lead">Search veterinary oncology hospitals by ZIP code, city, state or available service.</p><div class="controls"><input id="q" type="search" placeholder="Hospital, city or state" aria-label="Search by hospital, city or state"><input id="zip" inputmode="numeric" pattern="[0-9]{{5}}" maxlength="5" placeholder="ZIP code for nearest centers" aria-label="ZIP code for nearest centers"><select id="svc" aria-label="Filter by service">{options}</select></div><p class="note">This is a growing, non-exhaustive directory. Services were confirmed from official hospital sources, but availability may change. Contact the hospital directly before making plans.</p><p id="status" class="count" aria-live="polite"></p><p class="count"><span id="n">{len(centers)}</span> centers shown</p><div class="grid" id="cards">{cards}</div><details id="international" hidden><summary><span id="intl-n">0</span> international centers — distance not calculated</summary><div class="grid" id="intl-cards"></div></details><script>
