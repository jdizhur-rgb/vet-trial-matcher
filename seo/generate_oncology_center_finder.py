...
def portal(centers: list[dict]) -> str:
    ...
    return f'''<section class="portal"><link rel="stylesheet" href="../matcher-nav.css"><nav class="matcher-tools" aria-label="Finder sections"><a href="../">🐾 Clinical trials</a><a class="active" aria-current="page" href="../centers/">🏥 Oncology centers</a><a href="../advanced/">🧬 Advanced treatments</a><a href="../expanded-access/">🧪 Expanded access</a></nav><h1>Find veterinary oncology care near you</h1><p class="lead">Search for a veterinary oncologist, vet oncology clinic or cancer treatment center by ZIP code, city or state. Filter by available service to compare nearby options.</p><div class="controls"><input id="q" type="search" placeholder="Hospital, city or state" aria-label="Search by hospital, city or state"><input id="zip" inputmode="numeric" pattern="[0-9]{{5}}" maxlength="5" placeholder="ZIP code for nearest centers" aria-label="ZIP code for nearest centers"><select id="svc" aria-label="Filter by service">{options}</select></div><p class="note"><strong>Directory notice:</strong> Service information is compiled from clinic websites and professional public directories. Vet Trial Finder does not independently verify clinicians, licenses, credentials or quality of care, and a listing is not a recommendation. Confirm the clinician, service, cost and current availability directly with the clinic before making plans.</p><p id="status" class="count" aria-live="polite"></p><p class="count"><span id="n">{len(centers)}</span> oncology locations shown</p><div class="grid" id="cards">{cards}</div><details id="international" hidden><summary><span id="intl-n">0</span> international centers — distance not calculated</summary><div class="grid" id="intl-cards"></div></details><script>
...
def main() -> None:
    ...
    for path, canonical in OUTPUTS:
        ...
        rendered = re.sub(r'<title>.*?</title>', '<title>Find veterinary oncology care near you | Vet Trial Finder</title>', rendered, count=1)
        rendered = re.sub(r'<meta name="description" content="[^"]*">', '<meta name="description" content="Find veterinary oncology care, clinics and cancer treatment centers near you. Search by ZIP code, city or state and filter by service.">', rendered, count=1)
