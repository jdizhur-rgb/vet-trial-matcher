...
def main() -> None:
    ...
    for path, canonical in OUTPUTS:
        ...
        rendered = re.sub(r'<title>.*?</title>', '<title>Find a veterinary oncologist near you | Vet Trial Finder</title>', rendered, count=1)
        rendered = re.sub(r'<meta name="description" content="[^"]*">', '<meta name="description" content="Find a veterinary oncologist, vet oncology clinic or cancer treatment center near you. Search by ZIP code, city or state and filter by service.">', rendered, count=1)
