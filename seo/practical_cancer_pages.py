"""Turn diagnosis landing pages into short practical owner guides."""
...
def section(label,key,species):
 about,treatment,factors=CONTENT[key]; p=PRACTICAL[key]
 pet='dog' if species=='dogs' else 'cat'
...
<details><summary>The cancer has spread or cannot be removed</summary><div class="guide-detail"><p>Local treatment may still help with pain, bleeding, obstruction or another specific problem, but systemic treatment and trials often become more important. The realistic choices depend on the site, stage and your {pet}'s condition.</p></div></details>
<details><summary>My {pet} is already in treatment</summary><div class="guide-detail"><p>Trials may still be possible. Look for studies that allow the treatments already given, and check eligibility before changing therapy if the current treatment stops working.</p></div></details></div>
...
def apply_practical_cancer_guides(root:Path)->int:
 ...
 for path in root.rglob('index.html'):
  ...
  if len(rel)!=4 or rel[0] not in {'north-america','uk-europe'} or rel[1] not in {'dogs','cats'} or rel[3]!='index.html': continue
  key=rel[2].replace('-',' ')
  ...
  new,n=re.subn(r'<section class="disease(?: owner-guide)?">.*?</section>',section(label,key,rel[1]),text,count=1,flags=re.S)
 ...
 if not changed: raise AssertionError('Practical cancer guide matched no pages')
 print('PRACTICAL_CANCER_GUIDES_APPLIED',changed); return changed
