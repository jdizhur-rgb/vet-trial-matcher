...
function filterCenters(value){const q=value.toLowerCase().trim();const requestedState=stateNames[q]||(q.length===2&&stateCodes.has(q.toUpperCase())?q.toUpperCase():'');let count=0;centerCards.forEach(card=>{const states=(card.dataset.states||'').split(',');const show=!q||(requestedState?states.includes(requestedState):card.textContent.toLowerCase().includes(q));card.hidden=!show;if(show)count++;});document.querySelectorAll('.center-region').forEach(x=>{const show=!!x.querySelector('.center-directory-card:not([hidden])');x.hidden=!show;if(q&&show)x.open=true;});document.querySelectorAll('.center-directory-section').forEach(x=>{x.hidden=!x.querySelector('.center-directory-card:not([hidden])');});document.getElementById('center-search-status').textContent=q?(count?count+' matching '+(count===1?'center.':'centers.'):'No listed centers match that search.'):'';}
</script>'''
