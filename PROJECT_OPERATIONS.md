...
## 11b. Static matcher migration

- The no-sleep matcher is generated by `seo/static_matcher.py` at `/matcher/` and uses the same canonical `data/trials_base.json` catalog.
- Browser matching logic lives in `seo/matcher/matcher.js`; its conservative regression cases live in `scripts/test_static_matcher.cjs`.
- The generated public JSON contains only current, matchable treatment records and only owner-facing fields. Do not publish the complete internal catalog object to the browser.
- Keep `c-trials.streamlit.app` available as the rollback version until the static matcher has passed parity, mobile, link, and live deployment checks.
- A catalog change is incomplete until both the Streamlit smoke tests and `node scripts/test_static_matcher.cjs` pass during the migration period.
